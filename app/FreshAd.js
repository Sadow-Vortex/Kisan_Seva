...
export default function FreshAd() {
    ...
    const fetchFreshAds = async () => {
        try {
            ...
            console.log("Fresh ads load error:", e);
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    const openModal = async (ad) => {
        try {
            ...
            // 🔥 Try from cached users
            let farmer = users.find(user => user.id === updatedAd.advUserID);

            // 🔥 Fallback fetch if not found
            if (!farmer) {
                try {
                    const userRes = await axios.get(`${userUrl}/api/users/${updatedAd.advUserID}`);
                    farmer = userRes.data?.data;
                } catch (e) {
                    console.log("Farmer fallback fetch failed");
                }
            }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.subTitle}>Latest ads from farmers</Text>
                <Text style={styles.pageTitle}>Fresh Ads</Text>
            </View>
                            ...
                            <>
                                ...
                                <View style={styles.actionRow}>
                                    ...
                                    <TouchableOpacity
                                        ...
                                        onPress={() =>
                                            openDialer(selectedAd.farmer?.number)
                                        }
                                    >
